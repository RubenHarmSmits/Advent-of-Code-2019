...
public class Day183nd extends AoC {
    ...
    private void solve() throws IOException {
        ...
        makeAllOptions(grid);
        findFastestWay(new ArrayList(getStartKeys()),0,new ArrayList<>());

        log("Quickest way = "+quickest);
    }

    private ArrayList<Key> getStartKeys() {
        ArrayList<Key> currentKeys = new ArrayList<>();
        currentKeys.add(findKey("Å"));
        currentKeys.add(findKey("Ø"));
        currentKeys.add(findKey("Æ"));
        currentKeys.add(findKey("Ñ"));
        return currentKeys;
    }

    private void findFastestWay(ArrayList<Key> currentKeys, int steps, ArrayList<Key> keysFound) {
        if(steps<quickest){
            keysFound=updateKeysFound(keysFound,currentKeys);
            for(Key currentKey:currentKeys){
                ...
                for(Option option:filteredOptions){
                    findFastestWay(makeNewCurrentKeys(new ArrayList(currentKeys),currentKey,option.destination),steps+option.distance,new ArrayList(keysFound));
                }
            }
    ...
    private ArrayList<Key> updateKeysFound(ArrayList<Key> keysFound, ArrayList<Key> currentKeys) {
        for(Key cr:currentKeys){
            if(!keysFound.contains(cr)){
                keysFound.add(cr);
            }
        }
        return new ArrayList(keysFound);
    }

    private ArrayList<Key> makeNewCurrentKeys(ArrayList<Key> currentKeys, Key currentKey, Key destination) {
        ArrayList<Key> newCurrentKeys = new ArrayList(currentKeys);
        newCurrentKeys.remove(currentKey);
        newCurrentKeys.add(destination);
