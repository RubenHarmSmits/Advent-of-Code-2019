...
public class Day11 extends AoC{
    ...
    int[][] gridPainted = new int[length][length];
    int x=500;
    int y=500;
    Direction dir = Up;
    long outp1;
    long outp2;
    boolean outpIsFirst = true;
    ...
    void calculatePaintedPanels(){
        int count=0;
        for(int y=0;y<length;y++){
            for(int x=0;x<length;x++){
                if(gridPainted[y][x]==1){
                    count++;
                }
            }
        }
    ...
    void output(long output){
        ...
        outpIsFirst=!outpIsFirst;
    }

    void move(){
        grid[this.y][this.x] = outp1;
        gridPainted[this.y][this.x] = 1;
        switch(dir){
            case Up:
                if(outp2==0) {
                    dir = Left;
